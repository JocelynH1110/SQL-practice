...
--3.更新和刪除的指導原則
--使用 UPDATE 或 DELETE 時所遵循的重要原則：
--1.除非打算更新或刪除每一行，否則一定要使用 WHERE 子句。
--2.保證每個表都有主鍵（lesson 12 有內容說明），盡可能像 WHERE 子句那樣使用它（可以指定各主鍵、多個值或值的範圍）。
--在 UPDATE 或 DELETE 語句使用 WHERE 子句前，應先用 SELECT 進行測試，確保它過濾的是正確的紀錄，以防 WHERE 子句不正確。
--使用強制實施引用完整性的數據庫（lesson 12），DBMS將不允許刪除其數據或其他表相關的行。
--有的 DBMS 允許數據管理員約束，防止執行不帶 WHERE 子句的 UPDATE 或 DELETE 語句。若有的話，應該使用它。
