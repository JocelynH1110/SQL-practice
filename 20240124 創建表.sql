...
--3.刪除表：
--刪除整個表而不是其內容，語句為 DROP TABLE。
DROP TABLE if exists custCopy;

--Tip 使用關係規則防止意外刪除
--許多DBMS 許多允許強制實施有關規則，防止刪除與其他表相關連的表。
--若對某個表發布 DROP TABLE 語句，且該表是某個關係的組成部分，則 DBMS 將阻止這條語句執行，直到該關係被刪除為止。

--4.重新命名表：
--每個 DBMS 對重命名的支持有所不同。不存在嚴格的標準。
--postgreSQL 使用 RENAME 語句。
--所有重命名操作的基本語法都要求指定舊表名和新表名。具體語法參閱文檔。
